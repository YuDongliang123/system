...
ssize_t file_write(struct file *file, const char __user *uaf, size_t size, loff_t *offset)
{
	...
	{
		//用GPIOD开风扇
		FAN_init_control("1"); //传参
	}
	else
	{
		//用GPIOD关风扇
		FAN_init_control("2"); //传参
	}
...
int FAN_init_control(char abc[])
{
	...
	if (!(strcmp("1", abc)))
	{
		printk("---------------------转----------------------\n");
		//转
		desc = gpiod_get_from_of_node(enode, "fans", 0, GPIOD_OUT_HIGH, NULL);
		if (IS_ERR(desc))
		{
